...
play :: Board -> Disk -> Pos -> Maybe Board
...
placeDisk b d (x,y) = Board ( mtrx b !!= (x,row) )
  where row = ( mtrx b !! x) !!= (y, (Just d, (x,y)))

-- | flips the disks to given color in positions given
flipDisks :: Board -> Disk -> [Pos] -> Board
...
cellsToFlip :: Board -> Disk -> Pos -> [Pos]
cellsToFlip b d pos = cellsToFlip' b d pos (neighbours b d pos)

cellsToFlip' :: Board -> Disk -> Pos -> [Pos] -> [Pos]
cellsToFlip' b d (x,y) [] = []
...
neighbours :: Board -> Disk -> Pos -> [Pos]
neighbours b d (x,y) = filter (\x -> isLegal b x && ((getDisk b x) == (otherDisk d))) list
                        where
                         list = [(i,j) | i <- [x+1, x, x-1], j <- [y+1, y, y-1]]

-- | checks if the given position is a valid move for given color
isCandidate :: Board -> Disk -> Pos -> Bool
